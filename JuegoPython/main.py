...
#Colores (ya que utilice Figma  y Pixilart para la mayoria de las cosas algunos colores no se usaran en el codigo
#aun asi, sirven para modificicaciones y arreglos)
...
def ventana_default(screen):
    screen = pygame.display.set_mode((300,300))

#cronometro, se activa cuando se da el primer click
def tiempo(screen, text_font, primer_click, start_time):
    #Imagen de tiempo
...
def revelar_casillas_ady(tab, x, y, screen, celdas, sep):
        ...
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                ...
                if i != 0 or j != 0:
                    ...
                    revelar_casillas_ady(tab, x + i, y + j, screen, celdas, sep)

#funcion para imprimir todas las minas (perder)
def explotar(screen, celdas, sep):
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_g:
                    ...
                    estado_juego = 'ganar'
                    game_activo = False
                    flores(screen, celdas, sep)
                    tiempo_ganar = tiempo(screen, text_font, primer_click, start_time)
                    time.sleep(3)
                    ventana_default(screen)
                #Para perder
                if event.key == pygame.K_p:
                    #imprimo bombas y muevo mi estado a 'perder'
                    estado_juego = 'perder'
                    game_activo = False
                    explotar(screen, celdas, sep)
                    time.sleep(3)
                    ventana_default(screen)
            if game_activo:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if mouse_presses[0]:
                        ...
                        if primer_click:
                            ...
                            revelar_casillas_ady(tab,fila,col,screen, celdas, sep)
                            #Click en una mina
        ...
        if game_activo:
            if estado_juego == 'jugando':
                tiempo(screen, text_font, primer_click, start_time)
